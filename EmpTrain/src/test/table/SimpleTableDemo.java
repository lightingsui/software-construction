...
public class SimpleTableDemo extends JPanel {
	...
	public SimpleTableDemo() {
		...
		/*
		 * JTable还提供了一个重载的构造方法，闯入两个Vector JTable(Vector rowData, Vector
		 * columnNames)
		 */
		...
		// 这也是言方建议使用的方法，否则表头不会显示，需要单独获取到TableHeader自己手动地添加显示
		...
		JButton btn2 = new JButton("表格不调价整个视图(默认不填充)");
		panel2.add(btn1);
		panel2.add(btn2);
		...
		btn1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 设置表格跳虫整个视图，在默认情况下，如果表格的大小小于视图（窗体），
				// 你会发现下面的内容是其他颜色，可以将上面的yellow去掉做个比较
